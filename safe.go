...
func RunE(fn func() error, opt ...Option) (err error) {
	...
	defer func() {
		...
		if ex := recover(); ex != nil {
			...
			err = xerrors.New(fmt.Sprint(ex))
			stack := stack(3)
			if withJob != nil {
				withJob.(*Job).Ctx.SetStatus(StatusInternalError)
				jobAsJSON := []byte("")
				if j, err := json.Marshal(withJob); err == nil {
					// not an error, so set jobAsJSON
					jobAsJSON = j
				}
				fmt.Printf("[Recovery] %s panic recovered:\nJob: %s\n%s\n%s",
					timeFormat(time.Now()), jobAsJSON, err, stack)
			} else {
				fmt.Printf("[Recovery] %s panic recovered:\n%s\n%s",
